...
namespace Unity.Linq
{
    public static partial class GameObjectExtensions
    {
        ...
        public static void Destroy(this IEnumerable<Image> source, bool useDestroyImmediate = false, bool detachParent = false)
        {
            if (detachParent)
            {
                var l = new List<Image>(source); // avoid halloween problem
                var e = l.GetEnumerator(); // get struct enumerator for avoid unity's compiler bug(avoid boxing)
                while (e.MoveNext())
                {
                    e.Current.gameObject.Destroy(useDestroyImmediate, true);
                }
            }
            else
            {
                foreach (var item in source)
                {
                    item.gameObject.Destroy(useDestroyImmediate, false); // doesn't detach.
                }
            }
        }
